...
const search_api = async(req, res) => {
    try {
        ...
        for (var xyz in main_cat1) {
            const sub_cat = await Category.find({ parent_Id: main_cat1[xyz] }, ["name", "_id"])
            var objcat = {}
            objcat.name = main_cat[xyz].name
            objcat.Id = main_cat[xyz]._id
        ...
        if (!main_search && !city1 && !g_price1 && !l_price1 && !category1 && !subdivision_id1) {
            const allads12 = await advertise.find({})
        ...
        //If Price is Given
            ...
            const data_final1 = await advertise.find({
                ...
                price: { $gte: g_price1, $lte: l_price1 },
                category_id: { $in: lstdata },

            }).limit(limit).skip(skip);
            console.log(city3)
            console.log(reg1)
            console.log(g_price1, l_price1)
            console.log(lstdata)
            console.log(sublst)
...
const search_api1 = async(req, res) => {
    try {
        ...
        var lstdata = []
        if (category1) {
            const datafil = await Category.find({ name: category1 }, "_id")
            const childfil = await Category.find({ parent_Id: datafil }, "_id")
            for (var abcd in childfil) {
                lstdata.push(childfil[abcd]._id)
            }
...
module.exports = { getadvertise_pagination, search_api, get_sub_ads, getavailible, isdeleted, specificadvertise, updateadvertise, deleteadvertise, addadvertise }